...
function replaceIcons() {
    ...
    // Zastąp wszystkie elementy i-lucide odpowiednimi ikonami SVG
    document.querySelectorAll('i-lucide').forEach(icon => {
        ...
        if (iconMap[iconName]) {
            ...
            // Przenieś atrybuty stylu
            if (icon.hasAttribute('style')) {
                svgWrapper.setAttribute('style', icon.getAttribute('style'));
            }

            // Zastosuj rozmiar
            const size = icon.getAttribute('size');
            if (size) {
...
document.addEventListener('DOMContentLoaded', function() {
    // Zawsze wymuszaj użycie ikon zastępczych (nie próbuj używać Lucide)
    replaceIcons();
    ...
    if ('IntersectionObserver' in window) {
        ...
        document.querySelectorAll('.benefit-item, .about-content, .newsletter').forEach(item => {
